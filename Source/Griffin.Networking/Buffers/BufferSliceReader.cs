...
namespace Griffin.Networking.Buffers
{
    public class BufferSliceReader : ITextParser
    {
        ...
        {
            get { return _slice.RemainingCount <= 0; }
        }
        ...
        public char Peek
        {
            get { return Index < _slice.EndOffset - 1 ? (char) _slice.Buffer[Index + 1] : char.MinValue; }
        }
        ...
        public string ReadLine()
        {
            ...
            string thisLine = _encoding.GetString(_slice.Buffer, startIndex, Index - startIndex - 1);
            ...
            while (HasMore)
            {
                switch (Current)
                {
                    case '\\':
                        ...
                        break;
                    case '"':
                        string value = _encoding.GetString(_slice.Buffer, startPos, Index - startPos);
                        ++Index; // skip qoute
                        return value;
                    default:
                        Consume();
        ...
        {
            string str = _encoding.GetString(_slice.Buffer, Index, RemainingLength);
            Index = _slice.EndOffset;
            return str;
        }
