...
namespace Griffin.Core.Net.Protocols.Http.Implementation
{
    ...
    /// </summary>
    public sealed class HttpCookieCollection : IHttpCookieCollection<IHttpCookie>
    {
        private readonly IDictionary<string, HttpCookie> _items = new Dictionary<string, HttpCookie>();

        /// <summary>
        /// Let's copy all the cookies.
        /// </summary>
        /// <param name="cookies">value from cookie header.</param>
        public HttpCookieCollection(string cookies)
        {
            ...
            for (int i = 0; i < cookies.Length; ++i)
            {
                ...
                switch (state)
                {
                    ...
                    case 3:
                        ...
                        {
                            if (start != -1)
                                Add(new HttpCookie(name, cookies.Substring(start, i - start)));
                            start = -1;
                            state = 0;
                            name = string.Empty;
                        }
            ...
            if (start == -1)
                Add(new HttpCookie(name, string.Empty));
            else
                Add(new HttpCookie(name, cookies.Substring(start, cookies.Length - start)));
        }
        ...
        IEnumerator<IHttpCookie> IEnumerable<IHttpCookie>.GetEnumerator()
        {
            return _items.Values.GetEnumerator();
        }
