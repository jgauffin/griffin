...
namespace Griffin.Core.Net.Protocols.Http.Implementation.Headers
{
    ...
    public class HeaderParameterCollection
    {
        ...
        public static HeaderParameterCollection Parse(ITextParser reader)
        {
            return Parse(reader, ';');
        }
        ...
        /// <returns>A collection with all parameters (or just a empty collection).</returns>
        /// <exception cref="FormatException">Expected a value after equal sign.</exception>
        public static HeaderParameterCollection Parse(ITextParser reader, char delimiter)
        {
            ...
            while (name != string.Empty && !reader.EndOfFile)
            {
